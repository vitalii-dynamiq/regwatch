'use client';

import { FormFieldDatepicker } from '@/components/molecules/FormFieldDatepicker';
import { FormFieldInput } from '@/components/molecules/FormFieldInput';
import { FormFieldSelect } from '@/components/molecules/FormFieldSelect';
import { FormFieldTextarea } from '@/components/molecules/FormFieldTextarea';
import { emitFormPending } from '@/lib/helpers/formPending';
import { useAlertsPagination } from '@/server/api/entity/alert/queries';
import { useCreateObligation } from '@/server/api/entity/obligation/queries';
import { type ObligationCreateDto, createObligationSchema } from '@/server/api/entity/obligation/types';
import { useObligationAssets } from '@/server/api/entity/obligationAssets/queries';
import { useSourcesPagination } from '@/server/api/entity/source/queries';
import { useUsersPagination } from '@/server/api/entity/user/queries';
import { Form } from '@/ui/form';
import { zodResolver } from '@hookform/resolvers/zod';
import * as React from 'react';
import { useEffect } from 'react';
import { useForm } from 'react-hook-form';

interface ObligationCreateFormProps {
  formId?: string;
}

export default function ObligationCreateForm({ formId = 'obligationCreateForm' }: ObligationCreateFormProps) {
  const form = useForm<ObligationCreateDto>({
    resolver: zodResolver(createObligationSchema),
    defaultValues: {
      source_id: '',
      alert_id: undefined,
      title: '',
      obligation_type: '',
      content_type: '',
      description: '',
      risk_level: '',
      due_date: null,
      effective_date: null,
      assigned_to: undefined,
      regulatory_metadata: {},
    },
  });

  const { mutate, isPending } = useCreateObligation();

  React.useEffect(() => {
    emitFormPending(formId, isPending);
  }, [formId, isPending]);

  const { data: obligationAssets } = useObligationAssets();
  const { data: sourcesPagination } = useSourcesPagination({
    pageSize: 1000,
    orderBy: 'name',
  });
  const { data: usersPagination } = useUsersPagination({
    pageSize: 1000,
    orderBy: 'created_at',
  });
  const { data: alertsPagination } = useAlertsPagination({
    pageSize: 1000,
    orderBy: 'created_at',
  });

  const users = usersPagination?.items || [];
  const sources = sourcesPagination?.items || [];
  const alerts = alertsPagination?.items || [];

  const { risk_levels, obligation_types } = obligationAssets || {};

  const riskLevelOptions =
    risk_levels?.map((j) => ({
      value: String(j.id),
      textValue: j.name,
    })) ?? [];

  const obligationTypesOptions =
    obligation_types?.map((j) => ({
      value: String(j.id),
      textValue: j.name,
    })) ?? [];

  const usersOptions =
    users?.map((j) => ({
      value: String(j.id),
      textValue: `${j.first_name} ${j.last_name}`,
    })) ?? [];

  const sourcesOptions =
    sources?.map((j) => ({
      value: String(j.id),
      textValue: `${j.name}`,
    })) ?? [];

  const alertsOptions =
    alerts?.map((j) => ({
      value: String(j.id),
      textValue: `${j.title}`,
    })) ?? [];

  const selectedSourceId = form.watch('source_id');
  const selectedSource = sources.find((s) => s.id === selectedSourceId);
  const contentTypesOptions =
    selectedSource?.content_types?.map((ct) => ({
      value: String(ct.id),
      textValue: ct.name,
    })) ?? [];

  useEffect(() => {
    if (selectedSourceId) {
      form.setValue('content_type', '');
    }
  }, [selectedSourceId, form]);

  const onSubmit = (_values: ObligationCreateDto) => {
    mutate({ ..._values });
  };

  return (
    <Form {...form}>
      <form id={formId} onSubmit={form.handleSubmit(onSubmit)} className='space-y-8'>
        <FormFieldInput
          name='title'
          control={form.control}
          label='Title'
          placeholder='e.g. Market Risk Reporting'
          disabled={isPending}
        />
        <FormFieldTextarea
          label='Description'
          name='description'
          placeholder='Describe what needs to be done'
          control={form.control}
          disabled={isPending}
        />

        <div className='grid grid-cols-1 md:grid-cols-2 gap-4 items-start'>
          <FormFieldSelect
            label='Risk level'
            name='risk_level'
            control={form.control}
            options={riskLevelOptions}
            disabled={isPending}
          />
          <FormFieldSelect
            label='Obligation type'
            name='obligation_type'
            control={form.control}
            options={obligationTypesOptions}
            disabled={isPending}
          />
          <FormFieldSelect
            label='Source'
            name='source_id'
            control={form.control}
            options={sourcesOptions}
            disabled={isPending}
          />
          <FormFieldSelect
            label='Content type'
            name='content_type'
            control={form.control}
            options={contentTypesOptions}
            disabled={isPending || !selectedSourceId}
          />
          <FormFieldDatepicker
            label='Due date'
            name='due_date'
            control={form.control}
            disabled={isPending}
            placeholder='dd.mm.yyyy'
          />
          <FormFieldDatepicker
            label='Effective date'
            name='effective_date'
            control={form.control}
            disabled={isPending}
            placeholder='dd.mm.yyyy'
          />
          <FormFieldSelect
            label='Assignee'
            name='assigned_to'
            control={form.control}
            options={usersOptions}
            disabled={isPending}
          />
          <FormFieldSelect
            label='Alert'
            name='alert_id'
            control={form.control}
            options={alertsOptions}
            disabled={isPending}
          />
        </div>
      </form>
    </Form>
  );
}
